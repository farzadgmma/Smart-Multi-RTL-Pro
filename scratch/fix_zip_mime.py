...
try:
    ...
    if old_block in out:
        updated_nikai = out.replace(old_block, new_block)
        
        # Write to tmp and test
        with open(r'd:\extention Chrom\scratch\nikai_fix_mime.conf', 'w', encoding='utf-8') as f:
            f.write(updated_nikai)
        
        sftp = ssh.open_sftp()
        sftp.put(r'd:\extention Chrom\scratch\nikai_fix_mime.conf', '/tmp/nikai_fix_mime.conf')
        sftp.close()

        run_cmd("sudo cp /tmp/nikai_fix_mime.conf /etc/nginx/sites-enabled/nikai")
        ...
        if 'successful' in test_out or 'successful' in test_err:
            ...
            print('Nginx updated with Zip disposition!')
